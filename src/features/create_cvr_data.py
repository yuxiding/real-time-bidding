...
def create_cvr_training_data():
    ...
    conversions = pd.read_csv("data/raw/conversions.csv")
    ...
    # 5. 添加转化标签（只有点击过的用户才有可能转化）
    # 先标记点击
    merged_features['clicked'] = merged_features['request_id'].isin(clicks['request_id']).astype(int)
    # 再标记转化（只有点击过的用户才有可能转化）
    merged_features['converted'] = 0
    merged_features.loc[merged_features['clicked'] == 1, 'converted'] = merged_features.loc[merged_features['clicked'] == 1, 'request_id'].isin(conversions['request_id']).astype(int)
    ...
    # 7. 选择特征列
    # 排除非特征列
    exclude_cols = ['request_id', 'user_id', 'adslot_id', 'clicked', 'converted']
    feature_cols = [col for col in merged_features.columns if col not in exclude_cols]
    
    # 8. 保存训练数据
    output_file = "data/processed/cvr_training_data.csv"
    merged_features[feature_cols + ['converted']].to_csv(output_file, index=False)
...
if __name__ == "__main__":
    output_file, feature_dim = create_cvr_training_data()